<?php
namespace Modo\Gateway\Model\Service;
...
class OrderService
{
    ...
    public function purchaseAction($quoteId){
        ...
        if($result['success']) {
            $order = $result['order'];
            $order->setStatus($this->modoHelper->getNewOrderStatus());
    ...
    public function isOrderKeyValid($orderId,$orderKeyUnverified){
        ...
        try {
            ...
            $orderKey = (string)$order->getPayment()->getAdditionalInformation(\Modo\Gateway\Observer\DataAssignObserver::MODO_ORDER_KEY);
            if(isset($orderKey)){
    ...
    public function getPaymentIntention($pId){
        ...
        try{
           $response = $this->modoService->getPaymentIntention($pId);
    ...
    public function generatePaymentIntention($orderId){
        ...
        try{
            ...
            $order = $this->orderRepository->get($orderId);
            if(!$order->isCanceled()) {
                $this->modoHelper->log("StoreID->".   $order->getStoreId());
                $response = $this->modoService->createPaymentIntention( $order->getStoreId(),$order->getIncrementId(), $order->getTotalItemCount(), $order->getBaseGrandTotal());
    ...
    public function approveOrder($orderId){
        ...
        $order = $this->orderRepository->get($orderId);
        $order->setStatus($this->modoHelper->getApprovedOrderStatus());
    ...
    public function cancelOrder($orderId)
    {
        ...
        try{
             ...
             */
            $order = $this->orderRepository->get($orderId);
            if ($this->modoHelper->canCancelOnFailure()) {
                if ($order->canCancel()) {
                    $order->cancel();
                    $result['success'] = true;
            ...
            $order->addCommentToStatusHistory(__('Modo info: ') . $result['message']);
            $this->saveOrder($order);
            $result['quote_id'] = $order->getQuoteId();
        }catch (NoSuchEntityException $noSuchEntityException){
            $result['message'] = $noSuchEntityException->getMessage();
    ...
    public function getOrderToCancelCollection($storeId){
        ...
        $collection
            ...
            ->addFieldToFilter('main_table.status', ['eq' => $this->modoHelper->getNewOrderStatus()])
            ->getSelect()->join(

...
namespace Rollpix\Elebar\Console\Command;
...
use Rollpix\Elebar\Model\Service\OrderServiceFactory;
...
class CancelOrders extends Command
{
    ...
    {
        $this->setName('elebar:cancel:orders');
        $this->setDescription('Cancel Rollpix orders.');
        $this->addOption(
    ...
    public function cancelOrders(InputInterface $input, OutputInterface $output){
        if ($store_id = $input->getOption(self::STORE_ID_OPTION)) {
            ...
            $output->writeln("<error>Ingrese un StoreID valido para continuar. Por ejemplo: php bin/magento elebar:orders:cancel -s 1</error>");
            return \Magento\Framework\Console\Cli::RETURN_FAILURE;
        }
        $minutes = '15';
        $output->writeln("Cancelando ordenes de Rollpix creadas hace $minutes minutos");
            ...
            foreach ($orderCollection as $order) {
                ...
                if ($this->timezone->date(new \DateTime($order->getCreatedAt())) < $dateTimeCancelLimit) {
                    ...
                    if($cancelResponse['success']){
                        $output->writeln("Rollpix Cancel Orders: StoreId $store_id - $counter/$orderCollectionCount - #$incrementId - Cancelada exitosamente");
                        $ordersCanceledCount++;
                    }
                    else{
                        $output->writeln("<comment>Rollpix Cancel Orders: StoreId $store_id - $counter/$orderCollectionCount - #$incrementId - Con detalles</comment>");
                        $ordersWithErrors[] = $incrementId . ' ' . $cancelResponse['message'];
                    }
                } else {
                    $output->writeln("Rollpix Cancel Orders: StoreId $store_id - $counter/$orderCollectionCount - #$incrementId - Omitida");
                    $ordersSkippedCount++;
                }
                $counter++;
            }
    ...
    private function finish($output, $ordersCanceledCount, $ordersSkippedCount, $ordersWithErrors)
    {
        if($ordersCanceledCount == 0 && $ordersSkippedCount == 0 && count($ordersWithErrors) == 0){
            $output->writeln("<comment>No se encontraron ordenes de Rollpix para cancelar</comment>");
        }
        else {
            $output->writeln('');
